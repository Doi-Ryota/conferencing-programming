...
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # GPUは利用せずにCPUで計算
import sys
...
eval_env = gym.make(name_environment)
...
agt_prm = {
    #'n_act': env.n_act,
    'n_act': env.NUM_OF_CONVENTIONAL_METHODS,
    'filepath': (SAVE_DIR + '/' +
                 ENV_NAME + '_' +
                 agt_type)
}
...
sim_prm['n_step'] = None
sim_prm['n_episode'] = 100
sim_prm['eval_interval'] = 10
if (agt_type == 'fixedVMMethod' or
    agt_type == 'VMSweepingMethod' or
    agt_type == 'randomMethod' or
    agt_type == 'loadBalancingMethod'):
    # エージェントが強化学習でない場合は，時間を省くために学習期間と評価間隔を少なくする
    sim_prm['n_episode'] = 100
    sim_prm['eval_interval'] = 1
...
if (IS_LOAD_DATA is True) or \
    ...
    elif agt_type == 'fixedVMMethod':
        from agt_fixedVMMethod import FixedVMMethodAgt as Agt
    elif agt_type == 'VMSweepingMethod':
        from agt_VMSweepingMethod import VMSweepingMethodAgt as Agt
    elif agt_type == 'randomMethod':
        from agt_randomMethod import randomMethodAgt as Agt
    elif agt_type == 'loadBalancingMethod':
        from agt_loadBalancingMethod import loadBalancingMethodAgt as Agt
    ...
    # エージェントのインスタンス生成(G)
    print('Before generating an agent')
    agt = Agt(**agt_prm)
    #print('epsilon: {}'.format(agt.epsilon))
    print('n_act: {}'.format(agt.n_act))
    print('After generating an agent')
    #exit()
